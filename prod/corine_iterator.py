...
class MemoryTracker:
    def __init__(self, logger):
        ...
        self.min_estimated_max = float('inf')

        if self.has_psutil:
            self.process = psutil.Process()
            self.start_memory = self.process.memory_info().rss / 1024 / 1024  # MB
            self.system_total = psutil.virtual_memory().total / 1024 / 1024 / 1024  # GB
            self.system_available = psutil.virtual_memory().available / 1024 / 1024 / 1024  # GB

    def report_initial(self, change_count: int, revision_count: int):
        """Initial report with database info"""
        if not self.has_psutil:
            return

        total_input = change_count + revision_count
        self.log.msg(f"System Memory", f"{self.system_total:.1f} GB total, {self.system_available:.1f} GB available")
        self.log.msg(f"Input databases",
                     f"{revision_count:,} revision + {change_count:,} change = {total_input:,} polygons")
    ...
    def report(self, stage: str, poly_count: int = None):
        ...
        if poly_count and poly_count > 0:
            ...
            self.min_estimated_max = min(self.min_estimated_max, estimated_max_polys)
    ...
    def final_report(self):
        ...
        if self.min_estimated_max < float('inf'):
            self.log.iter(f"Estimated max capacity (worst case): {self.min_estimated_max:,} polygons")
...
class CorineGeneralizer:
    ...
    def run(self) -> str:
        ...
        # Get input counts for memory report
        change_count = int(arcpy.management.GetCount(self.cfg.input_change)[0]) if arcpy.Exists(
            self.cfg.input_change) else 0
        revision_count = int(arcpy.management.GetCount(self.cfg.input_revision)[0]) if arcpy.Exists(
            self.cfg.input_revision) else 0
        memory_tracker.report_initial(change_count, revision_count)
